...
public abstract class PalettedContainer$DataMixin<T> {
    ...
    public long[] initializeChunkPacketInfo(BitStorage storage, Operation<long[]> original, FriendlyByteBuf buf) {
        // Some mods serialize palette data outside Minecraft's normal chunk-packet scope.
        // In that case AntiXray has no packet metadata to update and must leave the data untouched.
        if (!Arguments.PACKET_INFO.isBound() || !Arguments.CHUNK_SECTION_INDEX.isBound()) {
